...
Status: Android M7 implementation and independent review passed; signed public
...
| Behavior | Desktop reference | Android M7 status |
| --- | --- | --- |
...
| Preview and generated-result history | `ui/main_window.py` | Implemented as persisted task history plus Gallery thumbnails and preview |
| Reuse task parameters | `ui/main_window.py` | Implemented from persisted Tasks entries |
| Reuse generated output as a reference image | `ui/main_window.py` | Implemented by copying the MediaStore result into the durable reference store |
...
These items are not part of the M7 implementation and must not be described as
currently supported:
...
- A permission-dialog Activity recreation can require the user to press Generate
  again; the selected draft asset remains available.
